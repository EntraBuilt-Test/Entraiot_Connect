...
const target = `.portfolio-item .card-car-overlay {
...
.portfolio-item .card-car-overlay .car-render-img {
  width: 100% !important;
...
.portfolio-item:hover .card-car-overlay .car-render-img {
  filter: saturate(1.1) brightness(1.05) drop-shadow(0 0 10px rgba(var(--dept-color-rgb), 0.4)) !important;
  transform: scale(1.06) !important;
}`;

const replacement = `.portfolio-item .card-car-overlay {
  position: absolute !important;
  top: 25% !important;
  left: 5% !important;
  width: 90% !important;
...
.portfolio-item:hover .card-car-overlay .car-render-img {
  filter: drop-shadow(0 0 18px rgba(var(--dept-color-rgb), 0.75)) !important;
...
files.forEach(file => {
  if (fs.existsSync(file)) {
    ...
    if (content.includes(normTarget)) {
      ...
      console.log('SUCCESS: Replaced sports car CSS overlay in', file);
    } else {
